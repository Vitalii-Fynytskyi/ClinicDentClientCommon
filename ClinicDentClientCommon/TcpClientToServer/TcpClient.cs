...
namespace ClinicDentClientCommon.TcpClientToServer
{
    ...
    public class TcpClient
    {
        ...
        public event ScheduleCabinetCommentUpdatedDelegate ScheduleCabinetCommentUpdated;
        public event Action<string> StagePayInfoUpdated;
        ...
        public TcpClient(Uri baseAddress, int port)
        {
            Uri uri = baseAddress;
            serverIp = uri.Host;
            serverPort = port;
        ...
        void SendToServer()
        {
            if (Connected == true)
                ...
                throw new Exception("З'єднання закрито. Дія неможлива");
        }
        ...
        public void ConnectToServer()
        {
            try
            {
                ...
                SetMessageToServerAndSend("login", SharedData.CurrentDoctor.EncodedJwt);
            }
            catch (Exception e)
            {
                socket?.Close();
        ...
        {
            StagePayInfoUpdated?.Invoke(updatedStagesContent);
        }
        private void answerScheduleCabinetCommentUpdated(string datetime, string cabinetId, string comment)
        {
            ScheduleCabinetCommentUpdated?.Invoke(datetime, Int32.Parse(cabinetId), comment);
        }
        ...
        private void answerWrongLogin()
        {
            ...
            throw new Exception("Не вдалось підключитись до сервера. Запустіть заново вікно");
        }
        ...
        private void answerClientOutOfDate()
        {
            ...
            throw new Exception("Ваша версія програми застаріла та не сумісна з сервером");
        ...
        public void UpdateCabinetComment(string dateTime, int cabinetId, string cabinetComment)
        {
